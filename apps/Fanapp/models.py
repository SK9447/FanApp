...
class UserManager(models.Manager):
    ...
    def login_validator(self, postData):
        ...
        if len(self.filter(email = postData['email'])) < 1:
            errors['email'] = 'Email & Password does not exist'
            return errors
        else:
...
class StoryManager(models.Manager):
    def story_validator(self, postData):
        errors = {}
        if len(postData['topic']) < 3:
            errors['topic'] = "Topic must be no fewer than 3 characters."
...
class Story(models.Model):
    ...
    created_at = models.DateTimeField(auto_now_add=True)
    user = models.ForeignKey(User, models.CASCADE, related_name="stories")
    objects = StoryManager()

class Marked_story(models.Model):
    topic = models.CharField(max_length=255)
    date_added = models.DateTimeField(auto_now=True)
    marked_at = models.DateTimeField(auto_now_add=True)
    likes = models.ManyToManyField(User, related_name='likes')
    user = models.ForeignKey(User, models.CASCADE, related_name="marked_stories")